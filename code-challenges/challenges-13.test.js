...
const longestString = (arr) => {
  ...
  let longest = 0;
  for (let elem of arr) {
    if (elem.length > arr[longest].length) {
      longest = arr.indexOf(elem);
    }
  }
  return longest;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(elem => elem.split(/\D/).join(''));
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((elem, idx) => idx % 2).join('');
};
...
const allHappy = (arr) => {
  return arr.every(elem => elem.includes(':)'));
};
